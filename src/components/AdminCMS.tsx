...
import { FileText, Save, Plus, ArrowLeft, LogOut, Loader2, Sparkles, CheckCircle2, AlertTriangle, Eye, Edit3 } from 'lucide-react';
...
function parseMarkdownToHtml(md: string): string {
  ...
  // Headers
  html = html.replace(/^### (.*$)/gim, '<h3 class="text-lg font-bold text-slate-100 mt-4 mb-2">$1</h3>');
  html = html.replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold text-slate-100 mt-5 mb-3 border-b border-slate-800 pb-1">$1</h2>');
  html = html.replace(/^# (.*$)/gim, '<h1 class="text-2xl font-extrabold text-slate-50 mt-6 mb-4">$1</h1>');
  ...
  html = html.replace(/```([\s\S]*?)```/g, '<pre class="bg-slate-950 p-4 rounded-lg my-4 overflow-x-auto border border-slate-800 text-xs font-mono text-cyan-400">$1</pre>');
  html = html.replace(/`(.*?)`/g, '<code class="bg-slate-950 px-1.5 py-0.5 rounded text-cyan-300 text-xs font-mono">$1</code>');
...
export function AdminCMS({ initialFiles, adminUser }: AdminCMSProps) {
  ...
  const [selectedFile, setSelectedFile] = useState<BlogFile | null>(initialFiles[0] || null);
  ...
  // Sign out handler
  const handleSignOut = () => {
    document.cookie = "admin_github_token=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC;";
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col">
      ...
      <header className="h-16 border-b border-slate-900 bg-slate-900/40 backdrop-blur px-6 flex items-center justify-between z-20">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent text-lg">Maybesoft CMS</span>
          <span className="text-xs bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full font-mono">v1.0.0</span>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <img src={adminUser.avatar_url} alt={adminUser.login} className="h-6 w-6 rounded-full ring-1 ring-purple-500/50" />
            <span className="text-xs text-slate-300 font-medium">@{adminUser.login}</span>
          </a>
          <button 
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 border-r border-slate-900 bg-slate-950/90 flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-1">
            {files.length === 0 ? (
              <div className="text-center py-8 text-slate-500 text-sm">No articles found. Create your first post!</div>
            ) : (
              files.map(file => {
        ...
        {selectedFile ? (
          <main className="flex-1 flex flex-col overflow-hidden bg-slate-900/20">
            ...
            <div className="h-14 border-b border-slate-900 px-6 flex items-center justify-between bg-slate-900/10 flex-shrink-0">
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-slate-300 font-mono line-clamp-1">{selectedFile.path}</span>
                {selectedFile.sha === '' && (
                  <span className="text-[10px] bg-amber-950/50 border border-amber-900 text-amber-400 px-2 py-0.5 rounded-full font-semibold uppercase tracking-wider animate-pulse">Draft</span>
                )}
              </div>
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className={`w-full md:w-1/2 flex flex-col overflow-y-auto p-6 space-y-6 ${activeTab === 'edit' ? 'block' : 'hidden md:block'} border-r border-slate-950`}>
                ...
                <div className="space-y-4 bg-slate-950/40 p-5 rounded-2xl border border-slate-900/60 shadow-inner">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="title" className="text-xs text-slate-400">Post Title</Label>
                      <Input 
                        id="title"
                        value={title} 
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="pubDate" className="text-xs text-slate-400">Publish Date</Label>
                      <Input 
                        id="pubDate"
                        type="date"
                        value={pubDate} 
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="description" className="text-xs text-slate-400">Meta Description</Label>
                    <Input 
                      id="description"
                      value={description} 
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="heroImage" className="text-xs text-slate-400">Hero Image URL</Label>
                    <Input 
                      id="heroImage"
                      value={heroImage} 
                ...
                <div className="flex-1 flex flex-col min-h-[400px]">
                  <Label htmlFor="editor-textarea" className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2 flex items-center gap-1.5">
              ...
              <div className={`w-full md:w-1/2 flex flex-col bg-slate-950/30 overflow-y-auto p-8 space-y-6 ${activeTab === 'preview' ? 'block' : 'hidden md:block'}`}>
                <div className="max-w-xl mx-auto w-full">
                  {/* Fake post header */}
                  {title && (
                    <header className="border-b border-slate-900 pb-5 mb-6">
                      {heroImage && (
                        <img src={heroImage} alt={title} className="w-full h-48 object-cover rounded-2xl mb-4 border border-slate-800 shadow" onError={(e) => (e.currentTarget.style.display = 'none')} />
                      )}
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-500">
                        ...
                        {description && <span className="text-slate-700">|</span>}
          ...
          <main className="flex-1 flex flex-col items-center justify-center bg-slate-900/10">
            <div className="text-center p-8 space-y-4 max-w-sm">
              ...
              <h2 className="text-xl font-bold text-white">No article selected</h2>
              <p className="text-slate-500 text-xs leading-relaxed">Select an article from the left sidebar to edit, or create a brand new draft to publish to your site.</p>
              <Button 
                onClick={handleCreateNew}
