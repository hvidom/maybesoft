...
export function DashboardPortal({ initialUser }: DashboardPortalProps) {
	...
	const [activeMenu, setActiveMenu] = useState<"overview" | "workspace">(
		"overview",
	);
	...
	const [clientTier, setClientTier] = useState<"free" | "basic" | "premium">(
		"free",
	);
	const [_isClientsLoading, setIsClientsLoading] = useState(false);
		...
		<div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:flex-row">
			...
			<main className="flex-1 flex flex-col min-w-0 bg-slate-950">
				...
				<header className="h-16 border-b border-slate-900 px-8 flex items-center justify-between">
					<h2 className="font-extrabold text-white tracking-tight capitalize">
						{activeMenu} Area
					</h2>
							...
							<div className="relative overflow-hidden rounded-3xl border border-slate-900 bg-slate-900/30 p-8 shadow">
								...
								<div className="relative z-10 space-y-2">
									...
									<p className="text-slate-400 text-sm max-w-lg leading-relaxed">
										...
										<strong className="text-cyan-300 font-semibold capitalize">
											{user.role}
										</strong>
										. Under the active workspace tab, you can manage your
										operations, view records, and configure settings.
									</p>
								</div>
							</div>
							...
							<div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
								<Card className="border-slate-900 bg-slate-900/20 backdrop-blur">
									...
									<CardContent className="flex items-center gap-3">
										...
										<span className="text-xl font-black capitalize text-slate-200">
											{user.role}
										</span>
									</CardContent>
								</Card>
							...
							{user.role === "lead" && (
								<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
										...
										<Card className="border-slate-800 bg-slate-900/40 shadow-xl rounded-2xl">
											<CardHeader>
												<CardTitle className="text-lg text-slate-100 flex items-center gap-2">
													<Plus className="h-5 w-5 text-cyan-400" /> Log
													Business Lead
												</CardTitle>
											</CardHeader>
											<CardContent>
												<form onSubmit={handleLeadSubmit} className="space-y-4">
													...
													<Button
														...
														className="w-full h-11 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl border-none shadow transition-all flex items-center justify-center gap-2"
													>
														{actionLoading ? (
									...
									<div className="space-y-4">
										...
										{isLeadsLoading ? (
											<div className="flex py-10 justify-center text-slate-500">
												<Loader2 className="animate-spin h-6 w-6 text-slate-600" />
											</div>
										) : leads.length === 0 ? (
											<div className="text-slate-500 border border-dashed border-slate-800/80 p-8 rounded-2xl text-center text-xs">
												No leads registered yet. Submit the form on the left!
											</div>
										) : (
											<div className="space-y-3.5">
												{leads.map((lead) => (
													<div
														key={lead.id}
														className="p-4 rounded-xl border border-slate-900 bg-slate-900/10 hover:border-slate-800 transition-all space-y-2.5"
													>
														...
														{lead.notes && (
															<div className="text-xs text-slate-500 bg-slate-950/40 p-2.5 rounded-lg italic leading-relaxed">
																{lead.notes}
															</div>
														)}
													</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
											...
											<CardContent>
												...
												>
													...
													<Button
														type="submit"
														disabled={actionLoading}
														className="w-full h-11 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl border-none shadow transition-all flex items-center justify-center gap-2"
													>
														{actionLoading ? (
															<Loader2 className="h-4 w-4 animate-spin" />
														) : (
									...
									<div className="space-y-4">
										...
										{isContactsLoading ? (
											...
											<div className="text-slate-500 border border-dashed border-slate-800/80 p-8 rounded-2xl text-center text-xs">
												No inquiries logged yet. Write a message on the left!
											</div>
										) : (
											<div className="space-y-3.5">
												{contacts.map((ticket) => (
													...
													>
														...
														<div className="text-xs text-slate-400 leading-relaxed bg-slate-950/40 p-2.5 rounded-lg">
															{ticket.message}
														</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
											...
											<CardContent>
												...
												>
													...
													<Button
														type="submit"
														disabled={actionLoading}
														className="w-full h-11 bg-purple-600 hover:bg-purple-500 text-white rounded-xl border-none shadow transition-all flex items-center justify-center gap-2"
													>
														{actionLoading ? (
															<Loader2 className="h-4 w-4 animate-spin" />
														) : (
									...
									<div className="space-y-6">
										<Card className="border-slate-800 bg-slate-900/30 rounded-2xl overflow-hidden relative">
											...
											<CardHeader>
												<span className="text-[10px] bg-purple-950 border border-purple-900 text-purple-400 px-2 py-0.5 rounded-full font-bold uppercase tracking-wider w-fit">
													ACTIVE TIER
												</span>
												<CardTitle className="text-2xl font-black capitalize text-slate-100">
													{clientTier} Membership
												</CardTitle>
											</CardHeader>
											<CardContent className="space-y-4 text-xs text-slate-400">
												<p className="leading-relaxed">
													Your account status has been updated in the local
													cloud architecture. With a{" "}
													<strong className="text-purple-300 capitalize">
														{clientTier}
													</strong>{" "}
													tier, you unlock advanced endpoints, API nodes, and
													optimized load timings.
												</p>
